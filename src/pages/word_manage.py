...
def render_word_manage_page():
    ...
    # 헤더 출력
    print(header_format.format("품사", "영어 철자", "한글 뜻", "난이도", "마지막 수정시각"))

    # 데이터 출력
    for word in words:
        print(row_format.format(word.type, word.english, word.korean, word.level, word.date_modified))
        
    
    operation_num = int(input("단어를 추가하려면 1, 단어를 수정하려면 2, 삭제하려면 3, 단어 관리를 취소하려면 0을 눌러주세요. : "))
    if operation_num == 1:
        add_id = len(words) + 1
        add_word = Word(id=None, english=input("영어단어를 입력해주세요 : "), korean=input("한글단어를 입력해주세요 : "), type=input("품사를 입력해주세요 : "), level=input("난이도를 입력해주세요 : "), date_modified=None, date_created=None, user_created=1)
        word_service.create(add_word)
        print("다음과 같이 추가되었습니다 : " 
            + word_service.get(add_id).english + " " 
            + word_service.get(add_id).korean + " " 
            + word_service.get(add_id).type + " " 
            + str(word_service.get(add_id).level) + " ")
      ...
      for word in words :
        if word.english == update_word :
          ...
          break
      else : print("수정하고싶은 단어가 단어장에 존재하지 않습니다.")
